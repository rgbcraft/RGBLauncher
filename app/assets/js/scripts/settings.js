    ...
    if (arguments_[0] === MSFT_REPLY_TYPE.ERROR) {
        ...
        switchView(getCurrentView(), viewOnClose, 500, 500, () => {
            ...
            setOverlayContent(
                ...
                'L\'auntenticazione Microsoft è fallita. Riprova.',
                'OK'
            )
            setOverlayHandler(() => {
                toggleOverlay(false)
                ...
                .catch((displayableError) => {
                    ...
                    if (isDisplayableError(displayableError)) {
                        ...
                        actualDisplayableError = {
                            title: 'Errore Sconosciuto Durante il Login',
                            desc: 'Un errore sconosciuto è stato appena trovato. Guarda la console per i dettagli.'
                        }
                    }
        ...
        val.onclick = (e) => {
            ...
            for (let i = 0; i < selectBtns.length; i++) {
                if (selectBtns[i].hasAttribute('selected')) {
                    ...
                    selectBtns[i].innerHTML = 'Seleziona Account'
                }
            }
        ...
        val.onclick = (e) => {
            ...
            if (Object.keys(ConfigManager.getAuthAccounts()).length === 1) {
                ...
                setOverlayContent(
                    ...
                    'Per usare il launcher devi essere loggato in almeno un account. Dovrai fare il login di nuovo.<br><br>Sei sicuro di voler fare il log out?',
                    'Sono Sicuro',
                    'Annulla'
                )
                setOverlayHandler(() => {
...
ipcRenderer.on(MSFT_OPCODE.REPLY_LOGOUT, (_, ...arguments_) => {
    if (arguments_[0] === MSFT_REPLY_TYPE.ERROR) {
        switchView(getCurrentView(), VIEWS.settings, 500, 500, () => {
            ...
            setOverlayContent(
                'Qualcosa È Andato Storto',
                'Logout Microsoft fallito. Riprova.',
                'OK'
            )
            setOverlayHandler(() => {
                toggleOverlay(false)
...
function refreshAuthAccountSelected(uuid) {
    Array.from(document.getElementsByClassName('settingsAuthAccount')).map((val) => {
        ...
        if (uuid === val.getAttribute('uuid')) {
            ...
            selBtn.innerHTML = 'Seleziona Account'
        }
    })
}
    ...
    authKeys.forEach((val) => {
        ...
        const accHtml = `<div class="settingsAuthAccount" uuid="${acc.uuid}">
            <div class="settingsAuthAccountLeft">
                <img class="settingsAuthAccountImage" alt="${acc.displayName}" src="http://skins.rgbcraft.com/api/helm/${acc.displayName}/1024">
            </div>
                        ...
                        <div class="settingsModMainWrapper">
                            ...
                            <div class="settingsModDetails">
                                ...
                                <div class="settingsDropinRemoveWrapper">
                                    <button class="settingsDropinRemoveButton" remmod="${dropin.fullName}">Rimuovi</button>
                                </div>
                            </div>
                        </div>
                        <label class="toggleSwitch">
    ...
    Array.from(sEls).map((v, index, arr) => {
        v.onclick = async () => {
            ...
            if (res) {
                ...
                setOverlayContent(
                    `Eliminazione Fallita<br>Mod Aggiuntiva ${fullName}`,
                    'Assicurati che il file non sia in uso e riprova.',
                    'OK'
                )
        ...
        if (dropinUI != null) {
            ...
            if (DropinModUtil.isDropinModEnabled(dropin.fullName) != dropinUIEnabled) {
                DropinModUtil.toggleDropinMod(CACHE_SETTINGS_MODS_DIR, dropin.fullName, dropinUIEnabled).catch(err => {
                    if (!isOverlayVisible()) {
                        setOverlayContent(
                            'Impossibile Abilitare/Disabilitare<br>Una o Più Mod Aggiuntive',
                            err.message,
                            'OK'
                        )
                        setOverlayHandler(null)
...
async function populateJavaExecDetails(execPath) {
    ...
    if (details != null) {
        settingsJavaExecDetails.innerHTML = `Selezionato: Java ${details.semverStr} (${details.vendor})`
    } else {
        settingsJavaExecDetails.innerHTML = 'Selezione Invalida'
    }
}

function populateJavaReqDesc(server) {
    settingsJavaReqDesc.innerHTML = `Richiede Java ${server.effectiveJavaOptions.suggestedMajor} x64.`
}

function populateJvmOptsLink(server) {
    const major = server.effectiveJavaOptions.suggestedMajor
    settingsJvmOptsLink.innerHTML = `Opzioni Disponibili per Java ${major} (HotSpot VM)`
    if (major >= 12) {
...
function populateVersionInformation(version, valueElement, titleElement, checkElement) {
    ...
    if (isPrerelease(version)) {
        titleElement.innerHTML = 'Versione Beta'
...
function populateReleaseNotes() {
    $.ajax({
        url: 'https://github.com/Enn3Developer/RGBLauncher/releases.atom',
        success: (data) => {
        ...
        settingsAboutChangelogText.innerHTML = 'Impossibile caricare le note di rilascio.'
    })
}
...
function populateSettingsUpdateInformation(data) {
    if (data != null) {
        settingsUpdateTitle.innerHTML = `Nuova ${isPrerelease(data.version) ? 'Versione Beta' : 'Versione Stabile'} Disponibile`
        ...
        if (process.platform === 'darwin') {
            settingsUpdateButtonStatus('Scarica da GitHub<span style="font-size: 10px;color: gray;text-shadow: none !important;">Chiudi il launcher ed esegui il dmg per aggiornare.</span>', false, () => {
                shell.openExternal(data.darwindownload)
            })
        } else {
            settingsUpdateButtonStatus('Scaricamento..', true)
        }
    } else {
        settingsUpdateTitle.innerHTML = 'Stai Usando l\'Ultima Versione'
        settingsUpdateChangelogCont.style.display = 'none'
        populateVersionInformation(remote.app.getVersion(), settingsUpdateVersionValue, settingsUpdateVersionTitle, settingsUpdateVersionCheck)
        settingsUpdateButtonStatus('Controlla Aggiornamenti', false, () => {
            if (!isDev) {
                ipcRenderer.send('autoUpdateAction', 'checkForUpdate')
                settingsUpdateButtonStatus('Controllando Aggiornamenti..', true)
            }
        })
    }
}
